...
public class KhachHangDao extends AbstractCRUD<KhachHang> {
	...
	public List<KhachHang> timKhachHangTheoCMND(String cmnd1){
		try {
			String sql="select kh from KhachHang kh  where kh.cmndKH like '%"+cmnd1+"%'";
			Query q=em.createQuery(sql,KhachHang.class);
//			q.setParameter("cmnd","%"+cmnd1+"%" );
...
//			q.setParameter("sdt","%"+sdt+"%" );
	...
	public boolean themKhachHang(KhachHang kh) {
		try {
			if(timKhachHangTheoMa(kh.getMaKhachHang())!=null) {
				return false;
			}
			else {
				persist(kh);
	...
	public boolean suaKhachHang(KhachHang kh) {
		try {
			KhachHang kh1=timKhachHangTheoMa(kh.getMaKhachHang());
			System.out.println(kh1.getMaKhachHang()+"ma");
			if(kh1!=null) {
